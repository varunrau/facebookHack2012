/*
	Takes the information and outputs a pretty graph using Graph Dracula
	and Raphael.js. 
	@param curUrl - the current url the user is on
	@param curTitle - the title of the current page
	@param childrenUrls - the children of this url (this will be an array)
	@param pageTitles - the titles of the children urls
	@return I have no idea what this thing is going to return
*/

function(curUrl, curTitle, childrenUrls, pageTitles) {
	var g = new Graph();
			i = 0;
			var image = "http://grabicon.com/" + curUrl + ".png";
			g.addNode("curUrl")
			var renderCur = function(r, n) {
				var set = r.set().push(
					r.image(image, 10, 10, 80, 80));
				return set;
			}
			
			var renderChild = function(r, n) {
				var set = r.set().push(
					r.image(image))
			}
