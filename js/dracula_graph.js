...
Graph.Renderer.Raphael = function(element, graph, width, height) {
	...
	this.dragger = function(e) {
		//alert(this.node.id);
		window.location = "http://" + this.node.id;
	}
...
if(counter == 0)
                var ellipse = r.ellipse(200, 50, 30, 20).attr({fill: color, stroke: color, "stroke-width": 2});
else
                var ellipse = r.ellipse(counter * 100, 200, 30, 20).attr({fill: color, stroke: color, "stroke-width": 2});
/* set DOM node ID */
                ellipse.node.id = node.label || node.id;
                shape = r.set().
                    push(ellipse).

	push(r.text(counter == 0 ? 200 : counter * 100, counter == 0 ? 80 : 230, node.label || node.id));
